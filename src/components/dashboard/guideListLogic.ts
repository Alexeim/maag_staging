...
export default (initialState: { apiBaseUrl: string }) => ({
  ...
  async confirmAndDelete(id: string) {
    try {
      const response = await fetch(this.buildApiUrl(`/api/guides/${id}`), {
        method: "DELETE",
      });
      if (!response.ok && response.status !== 204) {
        throw new Error(`Deletion failed with status: ${response.status}`);
      }
      this.notify("Путеводитель удалён");
      setTimeout(() => {
        window.location.reload();
      }, 1500);
    } catch (error) {
      console.error(error);
      this.notify("Не удалось удалить путеводитель. Попробуй ещё раз.", "error");
    }
  },
